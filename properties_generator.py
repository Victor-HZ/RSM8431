...
def make_random_property(property_id: int):
    location = random.choice(main.locations_pool)
    ptype = random.choice(main.types_pool)
    price = random.randint(100, 500)
    features = random.sample(main.features_pool, k=random.randint(2, 4))
    tags = random.sample(main.tags_pool, k=random.randint(2, 3))
    max_guests = random.randint(1, 10)
    penv = random.choice(main.environments_pool)
